# APEX Experts Page Builder Block Developer Guide

This guide details how to create and register custom UI blocks in the **APEX Experts** landing page builder. It details the file creation/modification workflow, theme styling tokens, reusable layout components, and animation patterns to maintain design consistency across the codebase.
...
The APEX Experts design system operates under two distinct theme backgrounds: **Dark Sections** (cinematic, tech-focused) and **Light Sections** (clean, executive focus).
...
| :-------------------- | :------------------------- | :------------- | :------------------------------------------------------- |
| **Dark Theme**  | `bg-bg` (`#0b1020`)    | `text-white` | Hero, Platform Architecture, Operational Governance, CTA |
| **Light Theme** | `bg-white` (`#f5f5f5`) | `text-black` | Strategic Problem Section, Strategic Trust Metrics       |
...
- **Colors:**
  - `bg-bg` / `text-bg` — Primary deep dark background
  - `bg-black` / `text-black` — Charcoal dark theme text / light theme background
  - `bg-white` / `text-white` — Ice white background / dark theme text
  - `bg-blue` / `text-blue` — Brand blue accent color (typically for light theme highlights)
  - `bg-cyan` / `text-cyan` — Brand glowing cyan accent (typically for dark theme glows and details)
- **Typography:**
...
- **Best Practices:** Only use in **Dark Theme** sections (`bg-bg`) to prevent color washed/flat looks.
...
Formats normal text strings and injects the official **APEX Experts** logo component automatically.

- **Usage:** `<BrandText text={description} />`
- **Action:** Splitting strings on `"APEX Experts"` and injecting `<FullLogo />`. Always pass CMS description text fields through this.
...
- **Usage:** `<LucideIcon name="Shield" className="h-5 w-5 text-cyan" />`
...
export const FeatureHighlightBlock: React.FC<FeatureHighlightType> = ({
  ...
  return (
    <section className="relative overflow-hidden bg-bg text-white py-24">
      ...
      <SectionReveal className="container relative z-10 mx-auto px-4">
        ...
        >
          ...
          {/* Icon */}
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-cyan/10 border border-cyan/20 mb-6">
            <LucideIcon name={icon || "Shield"} className="h-6 w-6 text-cyan" />
          </div>
