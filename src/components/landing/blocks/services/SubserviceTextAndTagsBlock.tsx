        ...
        <div className="flex flex-col lg:flex-row bg-black/50 border border-outline/30 rounded-[1.5rem] gap-8 p-6 lg:p-10 lg:justify-between lg:gap-4 lg:rounded-[2rem] lg:bg-black/30 text-white relative items-start lg:items-center">
          ...
          {tags && tags.length > 0 && (
            <motion.ul
              ...
              className="grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-x-2.5 lg:gap-y-4 lg:px-6 flex-1 w-full"
            >
              {tags.map(({ tag, id }, index) => (
                <motion.li
                  ...
                  className="rounded-full p-4 border border-outline/30 font-montserrat font-semibold text-base leading-[160%] text-white text-center  transition-all duration-300 cursor-default select-none"
                >
                  {tag}
                </motion.li>
              ))}
